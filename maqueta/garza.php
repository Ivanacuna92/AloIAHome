...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hectare-zone {
            position: absolute;
            border: 2px solid rgba(253, 97, 68, 0.3);
            transition: all 0.3s ease;
            cursor: pointer;
        ...
        .hectare-zone:hover {
            ...
            box-shadow: 0 0 15px rgba(253, 97, 68, 0.5);
        }

        .hectare-info-content {
            padding: 1rem;
        }
        ...
        .activity-item.completed {
            ...
            border-left: 3px solid var(--aloia-success);
        }

        .activity-item.pending {
            background: rgba(251, 146, 60, 0.1);
            border-left: 3px solid var(--aloia-warning);
        }
...
<body>
    <div class="app-container">
        ...
        <div class="sidebar">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item">
                    <a class="nav-link" onclick="showModule('mapa')">
                        <i class="fas fa-map-marked-alt"></i>
                        Mapa Interactivo
                    </a>
                </li>
        ...
        <div class="main-content">
            ...
            <!-- Mapa Module -->
            <div id="mapa-module" class="module hidden">
                <div class="page-header">
                    <h1 class="page-title">Mapa Interactivo</h1>
                    <p class="page-subtitle">Vista satelital con actividades por hectárea</p>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 300px; gap: 2rem;">
                    <div class="chart-container">
                        <div class="chart-header">
                            <h3 class="chart-title">Vista Satelital del Rancho</h3>
                        </div>
                        <div id="interactive-map" style="position: relative; width: 100%; height: 600px; overflow: hidden;">
                            <img src="asset1.png" alt="Vista satelital" style="width: 100%; height: 100%; object-fit: cover;" id="satellite-image">
                            <div id="map-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></div>
                        </div>
                    </div>
                    
                    <div class="chart-container">
                        <div class="chart-header">
                            <h3 class="chart-title">Información de Hectárea</h3>
                        </div>
                        <div id="hectare-info" style="padding: 1rem;">
                            <div style="text-align: center; color: #6b7280; padding: 2rem;">
                                <i class="fas fa-hand-pointer" style="font-size: 2rem; margin-bottom: 1rem;"></i>
                                <p>Pasa el cursor sobre una hectárea para ver su información</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
       ...
       let salesByMarketChart = null;
       let priceEvolutionChart = null;
       let ranchComparisonChart = null;
       ...
       function initSalesCharts() {
           // Destroy existing charts if they exist
           if (salesByMarketChart) salesByMarketChart.destroy();
           if (priceEvolutionChart) priceEvolutionChart.destroy();
           
           // Sales by Market Chart
           const salesByMarketCtx = document.getElementById('salesByMarketChart');
           if (salesByMarketCtx) {
               salesByMarketChart = new Chart(salesByMarketCtx, {
                   type: 'doughnut',
                   data: {
           ...
           if (priceEvolutionCtx) {
               priceEvolutionChart = new Chart(priceEvolutionCtx, {
                   type: 'line',
                   data: {
       ...
       function initRanchCharts() {
           ...
           if (ranchComparisonChart) ranchComparisonChart.destroy();
           
           // Ranch Comparison Chart
           const ranchComparisonCtx = document.getElementById('ranchComparison');
           if (ranchComparisonCtx) {
               ranchComparisonChart = new Chart(ranchComparisonCtx, {
                   type: 'radar',
                   data: {
       ...
       // Test the showModule function immediately
       console.log('showModule function defined:', typeof showModule);

        // Datos hardcodeados de hectáreas
        const hectareData = {
            'h1': {
                id: 'A-1',
                coordinates: { x: 25, y: 2.5, width: 24, height: 24 }, // porcentajes
                activities: [
                    { type: 'completed', icon: 'check', text: 'Siembra completada - 15 Nov', date: '15 Nov' },
                    { type: 'completed', icon: 'tint', text: 'Riego aplicado - 18 Nov', date: '18 Nov' },
                    { type: 'pending', icon: 'seedling', text: 'Fertilización programada', date: '25 Nov' }
                ],
            ...
            'h2': {
                id: 'A-2',
                coordinates: { x: 49, y: 19, width: 24, height: 17 },
                activities: [
                    { type: 'completed', icon: 'check', text: 'Preparación de suelo', date: '10 Nov' },
                    { type: 'pending', icon: 'seedling', text: 'Siembra programada', date: '22 Nov' },
                    { type: 'pending', icon: 'spray-can', text: 'Aplicación de herbicida', date: '24 Nov' }
                ],
                status: 'Preparado para siembra',
                crop: 'Papa Russet',
                planted: 'Pendiente'
            },
            'h3': {
                id: 'B-1',
                coordinates: { x: 25, y: 26.5, width: 24, height: 8.5 },
                activities: [
                    { type: 'completed', icon: 'check', text: 'Cosecha completada', date: '12 Nov' },
                    { type: 'completed', icon: 'truck', text: 'Transporte realizado', date: '13 Nov' },
                    { type: 'pending', icon: 'plow', text: 'Preparación para próximo ciclo', date: '30 Nov' }
                ],
                status: 'Cosechado',
                crop: 'Papa Atlantic',
                planted: '15 Ago 2024'
            ...
            'h4': {
                id: 'B-2',
                coordinates: { x: 25, y: 35, width: 23, height: 16 },
                activities: [
                    { type: 'completed', icon: 'tint', text: 'Sistema de riego instalado', date: '08 Nov' },
                    { type: 'completed', icon: 'seedling', text: 'Fertilización base aplicada', date: '14 Nov' },
                    { type: 'pending', icon: 'bug', text: 'Control de plagas', date: '26 Nov' }
                ],
                status: 'Crecimiento avanzado',
                crop: 'Papa Russet',
                planted: '20 Oct 2024'
            },
            'h5': {
                id: 'C-1',
                coordinates: { x: 73, y: 8, width: 20, height: 28 },
                activities: [
                    { type: 'completed', icon: 'seedling', text: 'Siembra de papa semilla', date: '05 Nov' },
                    { type: 'completed', icon: 'water', text: 'Instalación de goteo', date: '08 Nov' },
                    { type: 'pending', icon: 'chart-line', text: 'Monitoreo de crecimiento', date: '28 Nov' }
                ],
                status: 'Siembra especializada',
                crop: 'Papa Semilla Premium',
                planted: '05 Nov 2024'
            },
            'h6': {
                id: 'C-2',
                coordinates: { x: 5, y: 50, width: 18, height: 22 },
                activities: [
                    { type: 'pending', icon: 'tractor', text: 'Arado programado', date: '20 Nov' },
                    { type: 'pending', icon: 'spray-can', text: 'Fumigación preventiva', date: '22 Nov' },
                    { type: 'pending', icon: 'calendar', text: 'Planificación de siembra', date: '25 Nov' }
                ],
                status: 'En preparación',
                crop: 'Papa Industrial',
                planted: 'Diciembre 2024'
            }
        };
        ...
        function initInteractiveMap() {
            ...
            Object.keys(hectareData).forEach(hectareId => {
                const hectare = hectareData[hectareId];
                const zone = document.createElement('div');
                zone.className = 'hectare-zone';
                zone.id = hectareId;
                
                // Posicionar usando porcentajes
                zone.style.left = hectare.coordinates.x + '%';
                zone.style.top = hectare.coordinates.y + '%';
                zone.style.width = hectare.coordinates.width + '%';
                zone.style.height = hectare.coordinates.height + '%';
                
                // Eventos
                zone.addEventListener('mouseenter', () => showHectareInfo(hectareId));
                zone.addEventListener('mouseleave', () => hideHectareInfo());
                
                overlay.appendChild(zone);
            });
        }

        function showHectareInfo(hectareId) {
            const hectare = hectareData[hectareId];
            const infoPanel = document.getElementById('hectare-info');
            
            if (!hectare || !infoPanel) return;
            
            const activitiesHtml = hectare.activities.map(activity => `
                <div class="activity-item ${activity.type}">
                    <i class="fas fa-${activity.icon}"></i>
                    <div>
                        <div style="font-weight: 500; font-size: 0.875rem;">${activity.text}</div>
                        <div style="font-size: 0.75rem; color: #6b7280;">${activity.date}</div>
                    </div>
                </div>
            `).join('');
            
            infoPanel.innerHTML = `
                <div class="hectare-info-content">
                    <div style="border-bottom: 1px solid #e5e7eb; padding-bottom: 1rem; margin-bottom: 1rem;">
                        <h4 style="font-size: 1.1rem; font-weight: 600; color: #1f2937; margin-bottom: 0.5rem;">
                            Hectárea ${hectare.id}
                        </h4>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; font-size: 0.875rem; color: #6b7280;">
                            <div><strong>Cultivo:</strong> ${hectare.crop}</div>
                            <div><strong>Estado:</strong> ${hectare.status}</div>
                            <div style="grid-column: 1/-1;"><strong>Plantado:</strong> ${hectare.planted}</div>
                        </div>
                    </div>
                    
                    <div>
                        <h5 style="font-size: 0.9rem; font-weight: 600; margin-bottom: 0.75rem; color: #374151;">
                            Actividades
                        </h5>
                        ${activitiesHtml}
                    </div>
